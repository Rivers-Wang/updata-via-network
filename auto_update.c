...
#include <fat.h>
#include <asm/arch/platform.h>  /* added by wheatfa */
...
/*#define AU_DATA     "data" */  /*added by wheatfa */
...
#else
#define AU_FL_FIRMWARE_ST	0x0
//#define AU_FL_FIRMWARE_ND	0x7FFFF    /* 512K */   
#define AU_FL_FIRMWARE_ND	0xFFFFF    /* modify by wheatfa */
#define AU_FL_KERNEL_ST		0x100000  
#define AU_FL_KERNEL_ND		0x3FFFFF    /* 3M */
#define AU_FL_ROOTFS_ST		0x400000
#define AU_FL_ROOTFS_ND		0xFFFFFF  /* 16M */
/*#define AU_FL_DATA_ST		0xF00000*/  /* 1M for save usr config data,added by wheatfa */
/*#define AU_FL_DATA_ND		0xFFFFFF*/
#endif
...
#define IDX_KERNEL	1
#define IDX_ROOTFS	2
#define IDX_DATA	3  /* added by wheatfa */

/* max. number of files which could interest us */
//#define AU_MAXFILES 3
#define AU_MAXFILES 4 /* added by wheatfa */
...
long ausize[AU_MAXFILES] = {
	...
	/*(AU_FL_DATA_ND + 1) - AU_FL_DATA_ST,*/ /* added by wheatfa */
};
...
static int au_check_header_valid(int idx, long nbytes)
{
	...
	/*added by wheatfa begin */
   if ((idx == IDX_DATA) &&(hdr->ih_type != IH_TYPE_FILESYSTEM))
   {
		printf ("Image %s wrong type\n", aufile[idx]);
		ausize[idx] = 0;
		return -1;
	...
	/* added by wheatfa */
	#if 1
     printf("checksum:%d, ausize[idx]:%d\n",checksum, ausize[idx]);
	#endif
...
void touptek_pwm_update(void)
{
	writel(0x4,PWM3_CTRL);/* close PWM3*/
	writel(0x06,PERI_CRG14);/* set PWM_clk=24M*/
	writel(0x249f00,PWM3_CFG0);/*config period=0.1s*/
	writel(0x124f80,PWM3_CFG1);
	writel(0x5,PWM3_CTRL);/*open PWM3*/
}

void touptek_pwm_boot(void)
{
	writel(0x06,PERI_CRG14);/*set PWM_clk=24M*/
	writel(0x16E3600,PWM3_CFG0);/*config period=1s*/
	writel(0xB71B00,PWM3_CFG1);
	writel(0x5,PWM3_CTRL);/*open PWM3*/
}
/*
